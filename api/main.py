...
@app.get("/")
async def root():
    return {"message": "Welcome to Puls-Events RAG API. Use /ask to query the assistant."}

@app.post("/ask", response_model=AnswerResponse)
async def ask_question(request: QuestionRequest):
    if rag is None:
        raise HTTPException(status_code=503, detail="RAG system is not initialized. Please ensure the FAISS index exists.")
...
async def rebuild_index():
    ...
    try:
        ...
        from scripts.rebuild_index import (
            load_events, build_documents, split_documents, build_faiss_index,
            PROCESSED_DATA_PATH, FAISS_INDEX_PATH
        )
